...
get_action_menu (GtkWidget *widget)
{
  while (widget) {
    ...
    if (MATEWNCK_IS_ACTION_MENU (widget))
      return MATEWNCK_ACTION_MENU (widget);
			       ...
			       int index)
{
  ...
  name = matewnck_workspace_get_name (matewnck_screen_get_workspace (matewnck_window_get_screen (window),
				  index));
...
refill_submenu_viewport (MateWnckActionMenu *menu)
{
  ...
  screen = matewnck_window_get_screen (menu->priv->window);
  workspace = matewnck_screen_get_workspace (screen, 0);

  matewnck_window_get_geometry (menu->priv->window,
...
static void
matewnck_action_menu_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
matewnck_action_menu_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
      case PROP_WINDOW:
        g_return_if_fail (MATEWNCK_IS_WINDOW (g_value_get_pointer (value)));
...
matewnck_action_menu_class_init (MateWnckActionMenuClass *klass)
{
  ...
  object_class->set_property = matewnck_action_menu_set_property;
  object_class->finalize = matewnck_action_menu_finalize;
...
matewnck_action_menu_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (matewnck_action_menu_parent_class)->finalize (object);
}
...
matewnck_action_menu_new (MateWnckWindow *window)
{
  ...
  return g_object_new (MATEWNCK_TYPE_ACTION_MENU,
                       "window", window,
                       NULL);
}
