...
gboolean
_matewnck_get_cardinal (Window  xwindow,
                    Atom    atom,
                    int    *val)
{
  Atom type;
  ...
  *val = 0;
  ...
  type = None;
  result = XGetWindowProperty (_matewnck_get_default_display(),
			       xwindow,
			       atom,
			       0, G_MAXLONG,
...
int
_matewnck_get_wm_state (Window  xwindow)
{
  Atom type;
  int format;
  gulong nitems;
  ...
  wm_state = _matewnck_atom_get ("WM_STATE");
  retval = NormalState;
...
gboolean
_matewnck_get_window (Window  xwindow,
                  Atom    atom,
                  Window *val)
{
  Atom type;
...
gboolean
_matewnck_get_pixmap (Window  xwindow,
                  Atom    atom,
                  Pixmap *val)
{
  Atom type;
...
gboolean
_matewnck_get_atom (Window  xwindow,
                Atom    atom,
                Atom   *val)
{
  Atom type;
...
char*
_matewnck_get_text_property (Window  xwindow,
                         Atom    atom)
{
  XTextProperty text;
  char *retval;
  ...
  text.nitems = 0;
  if (XGetTextProperty (_matewnck_get_default_display(),
                        xwindow,
                        &text,
                        atom))
    {
...
static char*
_matewnck_get_string_property_latin1 (Window  xwindow,
                                  Atom    atom)
{
  Atom type;
  int format;
...
char*
_matewnck_get_utf8_property (Window  xwindow,
                         Atom    atom)
{
  Atom type;
  int format;
  ...
  utf8_string = _matewnck_atom_get ("UTF8_STRING");
...
gboolean
_matewnck_get_window_list (Window   xwindow,
                       Atom     atom,
                       Window **windows,
                       int     *len)
{
...
gboolean
_matewnck_get_atom_list (Window   xwindow,
                     Atom     atom,
                     Atom   **atoms,
                     int     *len)
{
...
gboolean
_matewnck_get_cardinal_list (Window   xwindow,
                         Atom     atom,
                         gulong **cardinals,
                         int     *len)
{
...
char**
_matewnck_get_utf8_list (Window   xwindow,
                     Atom     atom)
{
  Atom type;
  int format;
...
void
_matewnck_set_utf8_list (Window   xwindow,
                     Atom     atom,
                     char   **list)
{
  Atom utf8_string;
  ...
  XChangeProperty (_matewnck_get_default_display(),
		   xwindow,
                   atom,
...
{
  XSync (_matewnck_get_default_display(), False);
    ...
    case PropertyNotify:
      {
        ...
        if (screen != NULL)
        ...
        else
          {
            ...
            if (app)
              _matewnck_application_process_property_notify (app, xevent);
            
            if (window)
              _matewnck_window_process_property_notify (window, xevent);
          }
      }
      break;
    ...
    case ConfigureNotify:
      {
        ...
        if (window)
          _matewnck_window_process_configure_notify (window, xevent);
      }
      break;

    case SelectionClear:
      {
        _matewnck_desktop_layout_manager_process_event (xevent);
      }
      break;
      ...
      while (i < ScreenCount (display))
        {
          ...
          if (s != NULL)
            sn_display_process_event (_matewnck_screen_get_sn_display (s),
                                      xevent);
...
void
_matewnck_event_filter_init (void)
{
...
int
_matewnck_xid_equal (gconstpointer v1,
                 gconstpointer v2)
{
...
guint
_matewnck_xid_hash (gconstpointer v)
{
...
void
_matewnck_iconify (Window xwindow)
{
  Display *display;
...
void
_matewnck_deiconify (Window xwindow)
{
  ...
  if (gdkwindow)
    ...
    XMapRaised (_matewnck_get_default_display (), xwindow);
  _matewnck_error_trap_pop ();
}

void
_matewnck_close (Screen *screen,
	     Window  xwindow,
	     Time    timestamp)
{
  Display *display;
  ...
  xev.xclient.message_type = _matewnck_atom_get ("_NET_CLOSE_WINDOW");
  xev.xclient.format = 32;
  xev.xclient.data.l[0] = timestamp;
  xev.xclient.data.l[1] = _matewnck_get_client_type ();
...
void
_matewnck_keyboard_move (Screen *screen,
                     Window  xwindow)
{
  Display *display;
  Window   root;
  ...
  xev.xclient.display = display;
  xev.xclient.window = xwindow;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_WM_MOVERESIZE");
  xev.xclient.format = 32;
  ...
  xev.xclient.data.l[4] = _matewnck_get_client_type ();
...
void
_matewnck_keyboard_size (Screen *screen,
                     Window  xwindow)
{
  Display *display;
  Window   root;
...
void
_matewnck_change_state (Screen  *screen,
		    Window   xwindow,
                    gboolean add,
                    Atom     state1,
                    Atom     state2)
...
#define _NET_WM_STATE_TOGGLE        2    /* toggle property  */  
  ...
  xev.xclient.display = display;
  xev.xclient.window = xwindow;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_WM_STATE");
  xev.xclient.format = 32;
  xev.xclient.data.l[0] = add ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
  xev.xclient.data.l[1] = state1;
  xev.xclient.data.l[2] = state2;
  xev.xclient.data.l[3] = _matewnck_get_client_type ();
  xev.xclient.data.l[4] = 0;
...
void
_matewnck_change_workspace (Screen     *screen,
			Window      xwindow,
                        int         new_space)
{
  Display *display;
  ...
  xev.xclient.display = display;
  xev.xclient.window = xwindow;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_WM_DESKTOP");
  xev.xclient.format = 32;
...
void
_matewnck_activate (Screen *screen,
                Window  xwindow,
                Time    timestamp)
{
  Display *display;
  ...
  xev.xclient.display = display;
  xev.xclient.window = xwindow;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_ACTIVE_WINDOW");
  xev.xclient.format = 32;
  xev.xclient.data.l[0] = _matewnck_get_client_type ();
...
void
_matewnck_activate_workspace (Screen *screen,
                          ...
                          Time    timestamp)
{
  ...
  xev.xclient.message_type = _matewnck_atom_get ("_NET_CURRENT_DESKTOP");
  xev.xclient.format = 32;
...
void
_matewnck_change_viewport (Screen *screen,
		       int     x,
		       int     y)
{
  Display *display;
  ...
  xev.xclient.display = display;
  xev.xclient.window = root;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_DESKTOP_VIEWPORT");
  xev.xclient.format = 32;
...
void
_matewnck_toggle_showing_desktop (Screen  *screen,
                              gboolean show)
{
  Display *display;
  Window   root;
  ...
  xev.xclient.display = display;
  xev.xclient.window = root;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_SHOWING_DESKTOP");
  xev.xclient.format = 32;
...
char*
_matewnck_get_session_id (Window xwindow)
{
  Window client_leader;
  
  client_leader = None;
  _matewnck_get_window (xwindow,
                    _matewnck_atom_get ("WM_CLIENT_LEADER"),
                    &client_leader);

  if (client_leader == None)
    return NULL;

  return _matewnck_get_string_property_latin1 (client_leader,
                                           _matewnck_atom_get ("SM_CLIENT_ID"));
}

int
_matewnck_get_pid (Window xwindow)
{
  int val;

  if (!_matewnck_get_cardinal (xwindow,
                           _matewnck_atom_get ("_NET_WM_PID"),
                           &val))
    return 0;
  else
    return val;
...
char*
_matewnck_get_name (Window xwindow)
{
  char *name;
  
  name = _matewnck_get_utf8_property (xwindow,
  ...
  if (name == NULL)
    name = _matewnck_get_text_property (xwindow,
                                    XA_WM_NAME);
...
char*
_matewnck_get_icon_name (Window xwindow)
{
  char *name;
...
char*
_matewnck_get_res_class_utf8 (Window xwindow)
{
  char *res_class;

  _matewnck_get_wmclass (xwindow, &res_class, NULL);
                   ...
                   char **res_name)
{
  ...
  XGetClassHint (_matewnck_get_default_display (), xwindow,
                 &ch);
...
gboolean
_matewnck_get_frame_extents (Window  xwindow,
                         int    *left_frame,
                         int    *right_frame,
                         int    *top_frame,
                         int    *bottom_frame)
{
  ...
  _matewnck_get_cardinal_list (xwindow,
...
void
_matewnck_select_input (Window xwindow,
                    int    mask)
{
  GdkWindow *gdkwindow;
  ...
  if (gdkwindow)
    {
      ...
      XGetWindowAttributes (_matewnck_get_default_display (), xwindow, &attrs);
      mask |= attrs.your_event_mask;
    }
  
  XSelectInput (_matewnck_get_default_display (), xwindow, mask);
  _matewnck_error_trap_pop ();
}
  
/* The icon-reading code is copied
 * from marco, please sync bugfixes
 */
static gboolean
                     ...
                     int         *d)
{
  ...
  XGetGeometry (_matewnck_get_default_display (),
...
GdkPixbuf*
_matewnck_gdk_pixbuf_get_from_pixmap (GdkPixbuf   *dest,
                                  Pixmap       xpixmap,
                                  int          src_x,
                                  int          src_y,
                                  int          dest_x,
                  ...
                  Pixmap *mask)
{
  ...
  *mask = None;
  ...
  _matewnck_error_trap_push ();
  icons = NULL;
  result = XGetWindowProperty (_matewnck_get_default_display (), xwindow,
  ...
  if (type != _matewnck_atom_get ("KWM_WIN_ICON"))
    {
      XFree (icons);
      return;
    }
...
{  
  if (atom == _matewnck_atom_get ("_NET_WM_ICON"))
    icon_cache->net_wm_icon_dirty = TRUE;
  else if (atom == _matewnck_atom_get ("KWM_WIN_ICON"))
    icon_cache->kwm_win_icon_dirty = TRUE;
  else if (atom == _matewnck_atom_get ("WM_HINTS"))
    icon_cache->wm_hints_dirty = TRUE;
}
                  ...
                  int            ideal_mini_height)
{
  ...
  *mini_iconp = NULL;
  ...
  if (!_matewnck_icon_cache_get_icon_invalidated (icon_cache))
  ...
  if (icon_cache->origin <= USING_WM_HINTS &&
      icon_cache->wm_hints_dirty)
    {
      ...
      _matewnck_error_trap_push ();
      hints = XGetWMHints (_matewnck_get_default_display (), xwindow);
      _matewnck_error_trap_pop ();
      pixmap = None;
      mask = None;
      if (hints)
  ...
  if (icon_cache->want_fallback &&
    ...
    {
      _matewnck_get_fallback_icons (iconp,
                                ideal_width,
                                ideal_height,
                                mini_iconp,
                                ideal_mini_width,
...
void
_matewnck_get_fallback_icons (GdkPixbuf **iconp,
                          int         ideal_width,
                          int         ideal_height,
                          GdkPixbuf **mini_iconp,
                          int         ideal_mini_width,
...
void
_matewnck_get_window_geometry (Screen *screen,
			   Window  xwindow,
                           int    *xp,
                           int    *yp,
                           int    *widthp,
                           int    *heightp)
{
  ...
  _matewnck_get_window_position (screen, xwindow, xp, yp);
                                ...
                                int     height)
{
  ...
  xev.xclient.display = display;
  xev.xclient.window = xwindow;
  xev.xclient.message_type = _matewnck_atom_get ("_NET_MOVERESIZE_WINDOW");
  xev.xclient.format = 32;
...
void
_matewnck_get_window_position (Screen *screen,
			   Window  xwindow,
                           int    *xp,
                           int    *yp)
{
...
void
_matewnck_set_icon_geometry  (Window xwindow,
			  int    x,
			  int    y,
			  int    width,
			  int    height)
{
  ...
  XChangeProperty (_matewnck_get_default_display (),
		   xwindow,
		   _matewnck_atom_get ("_NET_WM_ICON_GEOMETRY"),
...
void
_matewnck_set_desktop_layout (Screen *xscreen,
                          int     rows,
                          int     columns)
{
  Display *display;
   ...
   * to matewnck_screen_try_set_workspace_layout.
   *
...
get_server_time (Window window)
{
  ...
  info.timestamp_prop_atom = _matewnck_atom_get ("_TIMESTAMP_PROP");
  info.window = window;

  XChangeProperty (_matewnck_get_default_display (), window,
		   info.timestamp_prop_atom, info.timestamp_prop_atom,
		   8, PropModeReplace, &c, 1);

  XIfEvent (_matewnck_get_default_display (), &xevent,
...
static void
_matewnck_free_layout_manager (LayoutManager *lm)
{
  _matewnck_error_trap_push ();
...
int
_matewnck_try_desktop_layout_manager (Screen *xscreen,
                                  int     current_token)
{
  Display *display;
  Window root;
  ...
  selection_atom = _matewnck_atom_get (buffer);
          ...
          if (current_token == lm->token)
            {
              if (owner == lm->window)
                ...
                { /* we lost the selection */
                  _matewnck_free_layout_manager (lm);
                  break;
                }
            }
          else
            return MATEWNCK_NO_MANAGER_TOKEN; /* someone else has it */
        }
  ...
  if (owner != None)
    return MATEWNCK_NO_MANAGER_TOKEN; /* someone else has the selection */
  ...
  lm->manager_atom = _matewnck_atom_get ("MANAGER");
  ...
  if (XGetSelectionOwner (display, lm->selection_atom) !=
    ...
    {
      g_free (lm);
      return MATEWNCK_NO_MANAGER_TOKEN;
    }
...
void
_matewnck_release_desktop_layout_manager (Screen *xscreen,
                                      int     current_token)
{
  Display *display;
  GSList *tmp;
...
gboolean
_matewnck_desktop_layout_manager_process_event (XEvent *xev)
{
  GSList *tmp;
  LayoutManager *lm;
